              ...
              {({ open }) => (
                <>
                  ...
                  <Transition
                    ...
                    leaveTo="opacity-0"
                  >
                    <Popover.Panel className="absolute -left-8 top-full z-30 mt-3 w-screen max-w-sm overflow-hidden rounded-lg bg-white shadow-lg ring-1 ring-gray-900/5">
                      <div className="absolute inset-0 top-1/2 bg-white shadow" aria-hidden="true" />
                      <div className="group relative flex flex-col gap-x-6 rounded-lg p-4 text-sm leading-6">
                        { collection?.items?.map((subItem, index) => {
                          return <div key={ index } className="hover:bg-indigo-50 w-full p-2 rounded-lg">
                            <Link href={`${COLLECTIONS_ROUTE}/${subItem.resourceId.replace('gid://shopify/Collection/', '')}`} prefetch={ false }>
                              <a>
